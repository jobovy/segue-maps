    ...
    if True:
        ...
        for jj in range(marglogl.shape[0]):
            for kk in range(marglogl.shape[1]):
                marglogl[jj,kk]= misc.logsumexp(logl[jj,0,0,kk,3:,:,:,:,:,:,:].flatten())
        ...
        for jj in range(marglogl.shape[0]):
            for kk in range(marglogl.shape[1]):
                if options.conditional:
                    marglogl[jj,kk]= misc.logsumexp(logl[jj,0,0,:,kk,:,:,:,:,:,:])-misc.logsumexp(logl[:,0,0,:,kk,:,:,:,:,:,:])
                else:
                    marglogl[jj,kk]= misc.logsumexp(logl[jj,0,0,:,kk,:,:,:,:,:,:])
        #Normalize
        alogl= marglogl-numpy.amax(marglogl)
        #Get hR range
        lnhr, lnsr, lnsz, rehr, resr, resz= approxFitResult(fehs[ii],afes[ii],
...
def plotsrsz_single(ii,options,args):
    ...
    if True:
        ...
        for jj in range(marglogl.shape[0]):
            for kk in range(marglogl.shape[1]):
                if options.conditional:
                    marglogl[jj,kk]= misc.logsumexp(logl[:,0,0,:,3:,jj,kk,:,:,:,:].flatten())-misc.logsumexp(logl[:,0,0,:,3:,jj,:,:,:,:,:].flatten())
                else:
                    marglogl[jj,kk]= misc.logsumexp(logl[:,0,0,:,3:,jj,kk,:,:,:,:].flatten())
        #Normalize
        alogl= marglogl-numpy.amax(marglogl)
        #Get ranges
        lnhr, lnsr, lnsz, rehr, resr, resz= approxFitResult(fehs[ii],afes[ii],
...
def plotPout_single(ii,options,args):
    ...
    if True:
        ...
        for jj in range(marglogl.shape[0]):
            marglogl[jj]= misc.logsumexp(logl[:,0,0,:,3:,:,:,:,jj,:,:].flatten())
        #Normalize
