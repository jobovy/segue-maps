...
into mydb.DBNAME from fGetNearbyObjEq(PLATERA,PLATEDEC,89.4) n, star p
WHERE n.objID=p.objID
and ((p.dered_r between 14.5 and 20.2) or (p.dered_g between 14.5 and 20.2))
and (p.dered_g-p.dered_r) between 0.2 and 0.75
and (p.flags & (dbo.fPhotoFlags('SATURATED')+dbo.fPhotoFlags('EDGE')+dbo.fPhotoFlags('INTERP_PSF')+dbo.fPhotoFlags('BADCOUNTS')))= 0
